# 大衍 dayan · 多术数确定性规则引擎 + 玄学 SFT 数据工厂

> 「大衍之数五十，其用四十有九」（《周易·系辞》）。大衍筮法为周易正统衍算之法，
> 「大衍求一术」更是秦九韶《数书九章》中的古代算法之名——玄学之衍算，尽入确定性的规则枢机。
...
现包含 **16 个术数引擎**，统一注册、统一 CLI、统一 SFT 产出，并自带训练后对拍与线上监控闭环。
...
# 批量生成多术数 SFT（默认 16 引擎 × 40 条，含 40% 工具调用样本）
...
`dayan_cast(engine, params)`、拿到引擎结果再解读：
...
### NVIDIA：Qwen3.8-27B（Unsloth QLoRA）
...
## 六、训练后验证与线上监控（observe 子包）

分四层，越靠前越自动化：

1. **引擎自检**：`dayan selftest`，保证"计算器"本身正确。
...
3. **训练后对拍（自动）**：用训练没见过的留出样本（seed 999）问模型，再和引擎真值比对关键事实。
   ...
   ```
   输出 `fact_hit_rate`（关键事实命中率，排盘类要求≈100%）、`full_match_rate`、免责覆盖率、
   P95 延迟、分术数明细；错例落 badcase 回流。默认仅允许本机/内网模型地址，
   公网服务需显式加 `--allow-public-url`。
4. **解读质量（人工/裁判）**：按 `assets/eval_rubric.md` 五维打分（D1 排盘一致…D5 表达），红线 0 容忍；
   另抽通用中文任务防能力退化。

**线上监控网关（推荐生产架构：引擎先算、模型只解读）**：

```bash
# 先起模型服务，再起网关；网关默认只监听本机 127.0.0.1
python scripts/serve_monitored.py --port 9000 --log evals/requests.jsonl
dayan report --log evals/requests.jsonl --markdown --out evals/daily.md   # 出日报
```
...
- 全部输出定位为**传统命理/玄学文化参考与娱乐**，统一附免责声明，不做灾祸、寿命、生死断言，
  不构成医疗/投资/法律建议。
