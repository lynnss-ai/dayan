...
| `liuyao` | 六爻纳甲 | A | core | 装卦、纳甲、世应、六亲六神、旬空、变卦（断卦交模型） |
| `ziwei` | 紫微斗数 | A | core | 命身宫、五行局、十四正曜、六吉星+禄存羊陀、生年四化、大限 |
| `qimen` | 奇门遁甲 | A | core | 拆补自动定局（或手动遁/局）+时柱，排天地盘、九星八门八神、值符值使 |
...
- `qimen` 已内置通行**拆补法**自动定局（节气三元局数表 + 日柱符头定三元，也可手动传遁/局）；
  置闰/茅山等流派未内置；中宫寄坤二宫已处理。历法正确性有 JDN 黄金锚点测试保障（`tests/test_engine_accuracy.py`）。
